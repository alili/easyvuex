...
const transfer = function transfer (store) {
  ...
  for (let item in store.actions) {
    actions[item] = async function (context, options) {
      // let path = context.rootState.route.path
      ...
      if (!store.actions[item].check || (store.actions[item].check && !!store.actions[item].check(context, options))) {
        try {
          if (store.actions[item].action && typeof store.actions[item].action === 'function') {
            ...
            res = await rp[method]({
              uri: link,
              json: true,
              headers: store.actions.headers,
              form: store.actions[item].setting ? store.actions[item].setting(context, options) : options,
              [method === 'post' ? 'body' : 'qs']: store.actions[item].setting ? store.actions[item].setting(context, options) : options
            })
          }
        } catch (err) {
          console.error(`${item} err:`, err)
        }
        // 执行请求后的回调函数
