  ...
  for (let item in store.actions) {
    actions[item] = async function (context, options) {
      ...
      let _commit = commit.bind(this, {
        root: options && options.__root || this,
        path: options && options.__path || this.state.route.path
      })
        ...
        try {
          if (store.actions[item].action && typeof store.actions[item].action === 'function') {
            ...
            res = await axios({
              ...
              url: link,
              json: true,
              headers: typeof store.actions[item].headers === 'function' ? store.actions[item].headers(options, _get) : store.actions[item].headers,
